...
def start_screen(stdscr):
    stdscr.clear()
    stdscr.addstr(3, 40, "SPEED TYPING TEST PROGRAMME. ", curses.color_pair(1))
    stdscr.addstr(5, 40, "To continue, Press any key...")
    stdscr.addstr(10, 40, "written by Thomas Brayovic")
    stdscr.refresh()
    stdscr.getkey()
...
#loads in the txt to be tested with.
def load_txt():
    with open("text.txt", "r") as f:
        lines = f.readlines()
        return random.choice(lines).strip()

#Words per minute calc
def wpm(stdscr):
    target_txt = load_txt()
    current_txt = []
    wpm = 0
